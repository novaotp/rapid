...
impl Request {
    ...
    /// - `Io` : If there was an error while reading from the reader.
    /// - `Utf8` : If the body is not valid UTF8.
    /// - `HttpMethod` : If the HTTP request contains an invalid HTTP method.
    /// - `ParseIntError` : If the value `Content-Length` value is not an int.
    /// - `TransfertEncoding` : If a value other than chunked for `Transfert-Encoding` was received.
    /// - `MalformedStartLine` : Expected three parts in the start line, but received more or less.
    pub fn parse<T: io::Read>(reader: &mut BufReader<T>) -> Result<Self, RequestParseError> {
        ...
        let body = if headers.contains_key("Transfer-Encoding") {
            if headers.get("Transfer-Encoding").map(|v| v.as_str()) == Some("chunked") {
                Some(read_body_as_chunked(reader)?)
            } else {
                return Err(RequestParseError::UnsupportedTransfertEncoding(
                    String::from("Only `Transfert-Encoding: chunked` is supported currently."),
                ));
            }
...
fn read_body_as_chunked<T: io::Read>(
    reader: &mut BufReader<T>,
    ...
    loop {
        ...
        println!("chunk : <{}>", len_buf.trim());
        let length = usize::from_str_radix(len_buf.trim(), 16)?;
        if length == 0 {
            break;
        }

        content_buf.clear();
        content_buf.resize(length, 0u8);
        reader.read_exact(&mut content_buf)?;

        buf.append(&mut content_buf);
    }
...
pub enum RequestParseError {
    ...
    /// Received a value other than `chunked` for `Transfert-Encoding`, which are currently unsupported.
    UnsupportedTransfertEncoding(String),
...
mod tests {
    ...
    #[test]
    fn test_read_body_as_chunked_valid() -> Result<(), RequestParseError> {
        let data = b"5\r\n\
                                Hello\r\n\
                                6\r\n\
                                 World\r\n\
        ...
        assert_eq!(
            read_body_as_chunked(&mut reader)?,
    ...
    #[test]
    fn test_read_body_as_chunked_stops_at_length_zero() -> Result<(), RequestParseError> {
        let data = b"5\r\n\
                                Hello\r\n\
                                0\r\n\
                                \r\n\
        ...
        assert_eq!(read_body_as_chunked(&mut reader)?, String::from("Hello"));

        Ok(())
    }

    #[test]
    fn test_read_body_as_chunked_fails_at_negative_length() -> Result<(), RequestParseError> {
        let data = b"-5\r\n\
                                Hello\r\n\
                                6\r\n\
                                 World\r\n\
        ...
        assert!(matches!(
            read_body_as_chunked(&mut reader),
            Err(RequestParseError::InvalidSize(_))
        ));
